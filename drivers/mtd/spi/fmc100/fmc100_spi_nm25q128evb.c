/*
 * fmc100_spi_nm25q128evb.c
 *
 * The Flash Memory Controller v100 Device Driver for vendor
 *
 * Copyright (c) 2021 CHANGJIANG Technologies Co., Ltd.
 *
...
/*****************************************************************************/
#define NM25Q128EVB_CR_4BYTE_MASK     0x1
...
/****************************************************************************/
static void spi_nm25q128evb_set_cmd(struct fmc_spi *spi, u8 cmd)
{
	unsigned int regval;
...
/****************************************************************************/
static int spi_nm25q128evb_entry_4addr(struct fmc_spi *spi, int enable)
{
	...
	if (enable) {
		spi_nm25q128evb_set_cmd(spi, SPI_CMD_EN4B);
		...
		/* reset cmd */
		spi_nm25q128evb_set_cmd(spi, SPI_CMD_FIRST_RESET_4ADDR);

		spi_nm25q128evb_set_cmd(spi, SPI_CMD_SECOND_RESET_4ADDR);
...
/****************************************************************************/
static void spi_nm25q128evb_set_op(struct fmc_spi *spi)
{
	unsigned int regval;
	struct fmc_host *host = (struct fmc_host *)spi->host;
...
*/
static int spi_nm25q128evb_qe_enable(struct fmc_spi *spi)
{
...
QE_END:
	...
	fmc_pr(QE_DBG, "\t* End SPI Nor NM25Q128EVB %s Quad.\n", str[op]);
